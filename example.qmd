...
```{=typst}
#show "Typst": set text(fill: gradient.radial(..color.map.rainbow))
```
...
This filter also supports adding a label to the output, such as below:

::: {.highlight label='tip-highlight'}
Look at how yellow this gets.
:::

We can then link to it @tip-highlight.
